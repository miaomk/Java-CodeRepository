...
public class QuestionController {
	...
	public @ResponseBody Object addQuestion(
		...
		if (files != null && files.length > 0) {
			...
			for (MultipartFile file : files) {
				try {
					fileUrl += FileUtil.uploadFile(file, "question") + ",";
	 ...
	 * @return
	 */
	@RequestMapping(value = "/question/modifyQuestion/{questionId}")
	public @ResponseBody Object modifyQuestion(
			HttpServletRequest request,
			HttpSession session,
			HttpServletResponse response,
			@RequestParam(value = "file", required = false) MultipartFile[] files,
		...
		int count;
		try {
			count = questionService.modifyQuestion(question);
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 * @return
	 */
	@RequestMapping(value = "/question/deleteQuestion/{questionId}")
	public @ResponseBody Object deleteQuestion(HttpServletRequest request,
			HttpSession session, HttpServletResponse response, @PathVariable("questionId") Integer questionId) {
		ResultInfo rsInfo = new ResultInfo();
	 ...
	 * @return
	 */
	@RequestMapping(value = "/question/getQuestionById/{questionId}")
	public @ResponseBody Object getQuestionById(HttpServletRequest request,
			HttpSession session, HttpServletResponse response, @PathVariable("questionId") Integer questionId) {
		ResultInfo rsInfo = new ResultInfo();
		...
		if (question == null) {
			...
			rsInfo.setData(new Question());
			rsInfo.setTotal(0);
			return rsInfo;
		}
		rsInfo.setData(question);
	 ...
	 * @return
	 */
	@RequestMapping(value = "/question/getQuestionList")
	public @ResponseBody Object getQuestionList(HttpServletRequest request,
			HttpServletResponse response) {
		ResultInfo rsInfo = new ResultInfo();
		...
		try {
			totalCount = questionService.countTotal(pageTool);
		} catch (Exception e) {
		...
		try {
			questionList = questionService.getQuestionList(pageTool);
		} catch (Exception e) {
		...
		if (questionList.size() == 0) {
			...
			rsInfo.setData(new ArrayList<Question>());
			return rsInfo;
		}
		rsInfo.setData(questionList);
	 ...
	 * @return
	 */
	@RequestMapping(value = "/question/deleteQuestionBatch")
	public @ResponseBody Object deleteBatch(HttpServletRequest request,
		...
		int count;
		try {
			count = questionService.deleteBatch(idArr);
		} catch (Exception e) {
			e.printStackTrace();
